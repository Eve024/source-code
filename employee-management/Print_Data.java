...
class Print_Data implements ActionListener{
    ...
    String emp_id,fname,lname,address,phone,email,education,post,age,dob,aadhar;
    JButton b1,b2;
    ImageIcon icon;

    Print_Data(String e_id){
        try{
            conn con = new conn();    //object of conn class
            String str = "select * from employee where ID = '"+e_id+"'";
            ResultSet rs= con.s.executeQuery(str);   //result stored in rs

            while(rs.next()){
                                            //getstring - to retrieve particualr value from database
                fname = rs.getString("fname");   //name retrieved and stored in name 
                ...
                education = rs.getString("qualification"); //qualification retrieved and stored in edu...
                post = rs.getString("post");
               // aadhar = rs.getString("aadhar");
                emp_id = rs.getString("ID");  //ID from database stored in emp_id
            }
        }catch(Exception e){
            e.printStackTrace();
        }
        ...
        id9.setLayout(null);
        ImageIcon img=new ImageIcon(ClassLoader.getSystemResource("Employee/Icons/print1.jpg"));
        id9.setIcon(img);

        id8 = new JLabel("Employee Details");
        id8.setBounds(60,10,300,50);
        f.add(id8);
        id8.setFont(new Font("serif",Font.BOLD,28));
        id9.add(id8);
        f.add(id9);

        id = new JLabel("Employee Id:");
        id.setBounds(50,70,160,30);
        id.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id);

        aid = new JLabel(emp_id);
        aid.setBounds(230,70,200,30);
